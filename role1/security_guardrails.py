...
2. SQL Injection: Input sanitization, keyword stripping, and parameterized query enforcement.
...
# Prompt Injection threat patterns (attempting to override LLM system prompts)
...
_SCRIPT_PRIMITIVES_RE = re.compile(r"\b(alert|eval|prompt|confirm|document\.cookie|window\.location)\s*\(", re.IGNORECASE)
...
def sanitize_input_text(text: str, max_length: int = MAX_QUERY_LENGTH) -> str:
    ...
    # 1. Normalize Unicode (NFC) and convert to string
    text = unicodedata.normalize("NFC", str(text))

    # 2. Hard length truncation to prevent DoS memory attacks
    ...
    # 6. Strip raw SQL control syntax if present in unparameterized context
...
def sanitize_sql_query_param(param: str) -> str:
    """
    Sanitizes string parameters intended for SQL database execution.
    Note: Always use parameterized queries (e.g. cursor.execute("SELECT ...", (param,)))
    in addition to input sanitization.
    """
    if not param:
        return ""
    cleaned = str(param).replace("'", "''").replace('"', '""').replace(";", "")
    return _SQL_INJECTION_RE.sub("", cleaned).strip()
...
def sanitize_file_path(base_dir: str | Path, user_path: str | Path) -> Path:
    ...
    try:
        ...
        raise RetrievalError(f"Path traversal security violation: path '{user_path}' escapes base directory '{base}'")
    return target
...
def filter_anti_hallucination(
    ...
    """
    Anti-Hallucination Filter: Removes low-confidence vector noise matches that cause
    downstream LLM hallucinations. Ensures only grounded, relevant chunks reach the LLM.
    """
    grounded_chunks = [c for c in chunks if c.score >= min_similarity_score]
...
def generate_provenance_citations(chunks: List[DocumentChunk]) -> List[Dict[str, Any]]:
    ...
    so downstream LLM responses can cite exact source documents and prevent ungrounded claims.
    """
    citations = []
