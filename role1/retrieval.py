...
class RetrievalIndex:
    ...
    def load(self):
        ...
        # Resolve offline cache dir: check explicit cache_dir, temp fastembed cache,
        # index_dir.parent/models, or data/models — in that priority order.
        ...
        # When we have a local cache, prevent HF Hub from hitting the network
        # on every startup just to check for model updates.
        os.environ.setdefault("HF_HUB_DISABLE_SYMLINKS_WARNING", "1")
        os.environ.setdefault("HF_HUB_DISABLE_SYMLINKS", "1")
        if effective_cache_dir and effective_cache_dir.exists():
            os.environ.setdefault("HF_HUB_OFFLINE", "1")

        kwargs = {"model_name": self._model_info.get("model_name", "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"), "threads": threads}
        if effective_cache_dir and effective_cache_dir.exists():
