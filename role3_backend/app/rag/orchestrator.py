...
# Interface: async def transcribe(audio_bytes: bytes, content_type: str) -> str
from app.stt.mock_stt import transcribe  # noqa: E402
...
class RAGService:
    ...
    async def run(self, transcript: str) -> RAGResponse:
        """Execute the RAG pipeline for a text (already-transcribed) query."""
        return await self._pipeline(transcript=transcript, stt_ms=0.0)

    async def run_voice(self, audio_bytes: bytes, content_type: str) -> RAGResponse:
        """Execute the STT step then the RAG pipeline for a voice request."""
        t0 = time.perf_counter()
        transcript = await transcribe(audio_bytes, content_type)
        stt_ms = _ms(t0)
        return await self._pipeline(transcript=transcript, stt_ms=stt_ms)

    # ------------------------------------------------------------------
    # Core pipeline
    # ------------------------------------------------------------------

    async def _pipeline(self, transcript: str, stt_ms: float) -> RAGResponse:
        """
